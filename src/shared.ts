...
export async function resolveVersion(versionInput: string, token: string): Promise<string> {
  ...
  if (versionInput === 'latest') {
    ...
    try {
      ...
      const fallbackClient = new HttpClient('prek-action')
      const response = await fallbackClient.getJson<{tag_name: string}>(
        'https://api.github.com/repos/j178/prek/releases/latest',
        buildHeaders('')
      )
      if (!response.result?.tag_name) {
...
export async function installPrek(version: string): Promise<string> {
  ...
  const toolArch = getToolCacheArch()
  ...
  try {
    ...
    const extractedPath = asset.archiveType === 'zip' ? await tc.extractZip(archivePath) : await tc.extractTar(archivePath)
    const binaryPath = await findBinary(extractedPath, asset.binaryName)
    const cacheDir = await stageBinary(binaryPath, asset.binaryName)
    const toolPath = await tc.cacheDir(cacheDir, 'prek', toolVersion, toolArch)
...
function getToolCacheArch(): string {
  return getToolCacheArchFor(process.arch)
}
...
/**
 * Finds the prek executable inside an extracted release archive.
 */
export async function findBinary(rootDir: string, binaryName: string): Promise<string> {
  const matcher = await glob.create(path.join(rootDir, '**', binaryName).replaceAll(path.sep, '/'))
  const matches = await matcher.glob()
  const binaryPath = matches.find(match => path.basename(match) === binaryName)
  if (!binaryPath) {
    throw new Error(`Could not find ${binaryName} in extracted archive ${rootDir}`)
  }
  return binaryPath
}

/**
 * Stages just the executable in its own directory so it can be cached as a simple tool directory.
 */
export async function stageBinary(binaryPath: string, binaryName: string): Promise<string> {
  const stageDir = path.join(os.tmpdir(), `prek-action-${Date.now()}-${Math.random().toString(16).slice(2)}`)
  await fs.mkdir(stageDir, {recursive: true})
  const destination = path.join(stageDir, binaryName)
...
async function buildCacheKey(workingDirectory: string): Promise<string> {
  ...
  return `prek-v1|${runnerOs}|${runnerArch}|${pythonLocation}|${hash}`
}
...
async function hashConfigFiles(workingDirectory: string): Promise<string> {
  const matcher = await glob.create(
    [
...
async function getPrekCacheDir(): Promise<string> {
  let output = ''
  const code = await exec.exec('prek', ['cache', 'dir', '--no-log-file'], {
    ignoreReturnCode: true,
    silent: true,
    listeners: {
      stdout: (data: Buffer) => {
