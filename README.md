...
| `prek-version` | Version or semver range to install from the bundled manifest, for example `0.2.30`, `0.3.x`, `<=1.0.0`, or `latest` | No | `latest` |
...
This action resolves `prek-version` from a bundled release manifest checked into the repository, including installable `.tar.gz` and `.zip` assets plus their SHA-256 digests. A scheduled workflow refreshes that manifest from `j178/prek` releases, rebuilds `dist`, and opens a pull request when new versions are published.
